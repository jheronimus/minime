...
1. **Standardize on OpenRC for both targets**:
   ...
   - Buildroot enables OpenRC via `BR2_INIT_OPENRC=y` in [common.config](../../packages/components/buildroot/external/configs/common.config).

2. **Single source of truth for init services**:
   - All shared service definitions live in [packages/components/boards/common/overlay/etc/init.d/](../../packages/components/boards/common/overlay/etc/init.d/).
   - Runlevel bindings live in `packages/components/boards/common/overlay/etc/runlevels/`.
...
3. **Prune Buildroot SysV compatibility scripts in post-build**:
   ...
   - [post-build.sh](../../packages/components/buildroot/external/scripts/post-build.sh) cleans up all legacy SysV scripts and `sysv-rcs` symlinks before generating `system.erofs`:
     ```sh
...
- **Single Source of Truth**: New services are added once under `packages/components/boards/common/overlay/etc/init.d/` and work immediately on both targets.
