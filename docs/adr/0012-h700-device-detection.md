...
2. **SARADC stick channels** — devices with analog sticks (rg35xx-h) wire them as `adc-joystick` over the SoC SARADC; the raw saradc channels can be probed for activity.
...
DDR3/DDR4, RGB stick LEDs, and the RGB LED (rg35xx-2024) are cross-checks, not discriminators.
...
Ship the panel firmware for every supported H700 panel (`firmware/panels/*.panel`, `CONFIG_EXTRA_FIRMWARE`). Six panel blobs ship today (rg28xx, rg34xx/-sp, rg35xx-plus, rg35xx-plus-rev6, rg35xx-sp-v2); without one, even a correctly detected panel fails to initialize.
