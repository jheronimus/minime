...
- [ ] Implement a `Wi-Fi.pak` for MinUI (tools pak), based on the old upstream optional wifi support but isolated
  - [THEORY] Upstream has a `skeleton/EXTRAS/Tools/rgb30/Wi-Fi.pak/launch.sh` + `wifi.txt` (+ `platform.c`/`api` hooks) from commit `472e03d` ("initial optional wifi support for rgb30"). Port it to the `minime` platform as a standalone Tools pak (per `PAKS.md`), keeping all wiring inside the pak or the `workspace/minime/` port dir — no shared MinUI codebase changes.
