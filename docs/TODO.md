...
## UI
- [ ] Streamline the RetroArch cores between MinUI and Allium
  - [THEORY] Maybe we can build the same cores once and just package them in MinUI and Allium, so Allium only has to build the frontend. Same make config everywhere, less wasted time
  - [THEORY] It seems like Allium actually supports the Roms folder naming scheme from MinUI? So maybe we can just use that one as global? No complicated code in genassets.sh for preloaded-roms or update.sh for switching between the UIs
