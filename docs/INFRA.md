...
- **Jobs** (identical in both, differing only in libc values / alpine-vs-buildroot target dir):
  ...
  - `build-os` (matrix over the `setup` board list) — runs `make components` then `make image update` (once per UI). Uses `minime/targets/{alpine,buildroot}`, per-distro ccache/DL dirs, the matching `ui-{libc}` artifact, and the matching upload path.
