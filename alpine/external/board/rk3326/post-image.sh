...
FINAL_IMG="${BINARIES_DIR}/minime-rk3326.img"
...
for boot_file in Image rk3326-anbernic-rg351v.dtb boot.scr; do
...
# Copy all other compiled RK3326 device tree binary files
copied_dtb_count=0
for dtb_file in "${BINARIES_DIR}"/rk3326-anbernic-*.dtb; do
	if [ -f "${dtb_file}" ] && [ "$(basename "${dtb_file}")" != "rk3326-anbernic-rg351v.dtb" ]; then
		cp -f "${dtb_file}" "${SYSTEM_STAGE}/"
		copied_dtb_count=$((copied_dtb_count + 1))
	fi
done

echo "Successfully copied core assets and ${copied_dtb_count} additional RK3326 device tree binaries."
...
sed -i 's/sp-rk3326.img/minime-rk3326.img/g' "${ROOTPATH_TMP}/genimage.cfg"
