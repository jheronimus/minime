...
prepare_vm:
ifeq ($(OS),Darwin)
	@orb list --running --quiet 2>/dev/null | grep -qx '$(ORB_MACHINE)' || ./scripts/mac/prepare.sh
endif
...
ifeq ($(OS),Darwin)
	@command -v orb >/dev/null 2>&1 && orb list --running --quiet 2>/dev/null | grep -qx '$(ORB_MACHINE)' && orb -m '$(ORB_MACHINE)' -u '$(ORB_USER)' sh -c "rm -rf $(BUILDROOT_OUTPUT_DIR)" || true
else
...
clean-vm:
ifeq ($(OS),Darwin)
	@command -v orb >/dev/null 2>&1 && orb list --quiet 2>/dev/null | grep -qx '$(ORB_MACHINE)' && orb delete '$(ORB_MACHINE)' || true
else
