...
.PHONY: help prepare shell buildroot defconfig image clean clean-vm copy_images prepare_vm FORCE

help:
	@printf '%s\n' \
		"minime firmware build orchestrator:" \
		"  make prepare            - Provision host (Linux) or OrbStack VM (macOS)" \
		"  make shell              - Open a terminal shell (VM on macOS, native on Linux)" \
...
prepare_vm:
	@$(if $(filter Darwin,$(OS)),orb list --running --quiet 2>/dev/null | grep -qx '$(ORB_MACHINE)' || $(PREPARE_SCRIPT),true)

prepare:
	@$(PREPARE_SCRIPT)

shell: prepare_vm
	@$(if $(filter Darwin,$(OS)),orb -m '$(ORB_MACHINE)' -u '$(ORB_USER)' -w '$(LINUX_ROOT)' sh,bash)
...
copy_images:
	@mkdir -p $(CURDIR)/out; echo "Copying built firmware images to out/..."
	@$(if $(filter Darwin,$(OS)),orb -m '$(ORB_MACHINE)' -u '$(ORB_USER)' sh -lc "mkdir -p $(BUILDROOT_OUTPUT_DIR)/images && cp -r $(BUILDROOT_OUTPUT_DIR)/images/* $(LINUX_ROOT)/out/ 2>/dev/null || true",mkdir -p $(BUILDROOT_OUTPUT_DIR)/images && cp -r $(BUILDROOT_OUTPUT_DIR)/images/* $(ROOT_DIR)/out/ 2>/dev/null || true)

clean:
	@rm -rf $(BUILDROOT_DIR) $(LOG_DIR) out/
	@$(if $(filter Darwin,$(OS)),command -v orb >/dev/null 2>&1 && orb list --running --quiet 2>/dev/null | grep -qx '$(ORB_MACHINE)' && orb -m '$(ORB_MACHINE)' -u '$(ORB_USER)' sh -c 'rm -rf $(BUILDROOT_OUTPUT_DIR)' || true,rm -rf $(BUILDROOT_OUTPUT_DIR))

clean-vm:
	@$(if $(filter Darwin,$(OS)),command -v orb >/dev/null 2>&1 && orb list --quiet 2>/dev/null | grep -qx '$(ORB_MACHINE)' && orb delete '$(ORB_MACHINE)' || true,echo "clean-vm is only applicable on macOS (Darwin)." >&2)
