...
- **Build Pipeline Trigger Model**: The build pipeline is parameter-driven via **`build.yml`** (`libc`: `alpine` or `buildroot`, `soc`: `rk3566`, `rk3326`, or `h700`, `ui`: `muos`, `minui`, or `allium`) feeding the `testing` release. It triggers on every push to `main` filtered to `minime/**`, `src/**`, `.github/workflows/**` (building the default `alpine / rk3566 / muos`), `workflow_dispatch` (single selected target), and **`nightly.yml`** (daily cron at 04:00 UTC = 07:00 GMT+3 running a 15-target matrix over all valid combinations, excluding `buildroot + h700`). Each run calls the shared **`build-bootloader.yml`** reusable workflow (U-Boot for all three boards, cache-hit cheap). Concurrency is keyed per-target (`minime-build-${{ github.ref }}-${{ libc }}-${{ soc }}-${{ ui }}` with `cancel-in-progress: true`) — **the newest run wins** for the same target; disjoint targets run concurrently.
