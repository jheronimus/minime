...
- **Doc Size Limit (5 KB)**: No markdown doc in this repo may exceed **5 KB**. When a doc grows past the limit, edit it down without losing important details — prefer linking to the canonical source (traits files, code, DTS, other docs) over inlining tables or long prose. Each ADR covers exactly **one topic**; when a topic is addressed across several ADRs, merge them into a single ADR.
