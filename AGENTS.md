...
- **`build.yml` Trigger Model**: `build.yml` runs on three triggers — every push to `main` filtered to `minime/**` (fast path: only the target in the `FAST_PATH_TARGET` env at the top of `build.yml`, default `alpine/rk3566`, for both UIs), a daily cron at 04:00 UTC (`0 4 * * *`, = 07:00 GMT+3) that rebuilds every supported target to refresh `testing` images and catch regressions, and `workflow_dispatch` (comma-separated `targets` input, `all` = everything, e.g. `alpine/rk3566, buildroot/rk3326`). A `concurrency` group (`cancel-in-progress: true`) keyed on `github.ref` enforces a single active build per branch — **the newest run wins**. **If your run was cancelled, that is not an error**: a newer commit, dispatch, or the nightly cron superseded it. Do not retry or re-dispatch the cancelled run; look at the newest run instead. Only the default fast-path target can be rerouted by editing `FAST_PATH_TARGET` in `build.yml` — nothing else needs to change.
