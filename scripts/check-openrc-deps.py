...
def main():
    init_dir = Path("alpine/board/common/overlay/etc/init.d")
    if not init_dir.exists():
        print("OpenRC init.d directory not found — skipping.")
        sys.exit(0)

    init_files = [
        f for f in init_dir.iterdir() if f.is_file() and not f.name.startswith(".")
    ]
                ...
                if m:
                    ...
                    for t in targets:
                        ...
                        if t_clean not in defined_services:
                            print(
                                f"  [ERROR] {script.name}: '{kind}' references unknown service '{t_clean}'"
                            )
                            errors += 1
    ...
    if errors > 0:
        ...
        print(
            "OpenRC dependency check passed cleanly. All service dependencies resolve."
        )
        sys.exit(0)
