...
define MINIME_COPY_DTS
	if [ -d $(BR2_EXTERNAL_MINIME_PATH)/../../../boards/h700/dts ]; then \
		cp $(BR2_EXTERNAL_MINIME_PATH)/../../../boards/h700/dts/*.dts \
			$(LINUX_DIR)/arch/arm64/boot/dts/allwinner/; \
	fi
	if [ -d $(BR2_EXTERNAL_MINIME_PATH)/../../../boards/rk3326/dts ]; then \
		cp $(BR2_EXTERNAL_MINIME_PATH)/../../../boards/rk3326/dts/*.dts \
...
define MINIME_PATCH_LINUX_CONFIG
	...
	cp -rL $(BR2_EXTERNAL_MINIME_PATH)/../../../boards/common/firmware/* $(LINUX_DIR)/minime-firmware/
	if [ -d $(BR2_EXTERNAL_MINIME_PATH)/../../../boards/$(MINIME_BOARD_NAME)/firmware ]; then \
		cp -rL $(BR2_EXTERNAL_MINIME_PATH)/../../../boards/$(MINIME_BOARD_NAME)/firmware/* $(LINUX_DIR)/minime-firmware/; \
	fi
	sed -i 's|__MINIME_BOARD_FIRMWARE_DIR__|$(LINUX_DIR)/minime-firmware|g' $(LINUX_DIR)/.config
	sed -i 's|__MINIME_COMMON_FIRMWARE_DIR__|$(BR2_EXTERNAL_MINIME_PATH)/../../../boards/common/firmware|g' $(LINUX_DIR)/.config
endef
